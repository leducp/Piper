...
{
    // Phase advances by `dt` per compute() call. The host owns dt;
    // the engine has no clock of its own. If the node is active on
    // N stages, the wave steps N times per period -- by design, so
    // multi-stage tagging scales the effective rate.
    template<typename T>
