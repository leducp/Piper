...
A textbook AM radio chain -- modulator + noisy channel + envelope-
detector receiver -- with one probe per measurement point so the
time/freq plot tells the whole story.
...
`probe_carrier` taps `carrier_gen.out` directly. Six probes total,
one per stage of the chain.
...
The binary records every `external_output<float>` it finds, so the
CSV gets six probe columns; `plot.py` renders one row per column with
time series on the left and FFT magnitude (log y) on the right.
...
Hand-editing in the editor is fine for tweaks; topology changes
